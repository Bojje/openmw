...
BSShaderNoLighting materials now retain an explicit neutral unlit mode, so Vulkan does not apply
world lighting to authored unlit meshes.
NIF texture transforms now use independent neutral UV payloads for dark, detail, decal, normal,
glow, and gloss layers rather than implicitly reusing the base coordinates.
Bethesda effect-shader UV offset/scale transforms now cross the neutral material boundary and
are applied once during mesh batching, matching the legacy shared-UV transform without a scene-graph
texture matrix.
...
point-light color and radius through a bounded scene-light array consumed by Vulkan. Ordinary ESM3/ESM4 light
objects now use the same neutral position/color/radius payload, and their flicker/pulse controller state is evaluated
by the neutral world clock before submission; negative-light subtraction is preserved through the signed neutral light
position and Vulkan composite path. Enchanted neutral arrows
...
The neutral scene clears both caches at world reset and at each cell-grid shift; predicted bounds
also asynchronously prewarm one bounded set of LOD-0 cell tiles at a time, while higher LODs remain
demand-driven; reset waits for the active preload task before invalidating cell data. `WorldScene` now
...
231 code files excluding this ledger, deleting 2,575 lines and adding 19,640 lines (net `+17,065`). The larger Vulkan-only
...
| NIF-to-neutral mesh conversion | Renderer-neutral NIF boundary, material data, mesh cache, authored gloss texture transport, dark/detail/decal texture-layer composition with independent dark/detail/decal/normal/glow/specular transforms, Bethesda effect-shader UV offset/scale transport, static particle quad rotation and authored-color transport, neutral initial particle position/rotation/lifetime advancement, modern controller-owned particle vertex-code mapping, gravity/drag/rotation/grow/fade particle payload, bounded deterministic box-emitter spawn snapshots, planar/spherical collider reflection, ordered collider-chain traversal, die-on-collision, bounded collision-spawn and multi-generation spawn snapshots, skinning metadata, world-selected layered actor/additional `.kf` pose sampling, highest-priority group metadata, neutral animation-group handoff, scripted queue priority, per-bone neutral overlay masks and priorities, text-key extraction and selected-segment rebasing, neutral Lua/sound/melee/spell event dispatch with footstep and random-attack fallback parity, dynamic mesh payloads, named NPC body-part and armor/clothing attachments, neutral controller chains and transform blends, neutral weapon/shield attachments with equipment glow metadata, `SceneSubmission`, Vulkan mesh batch, full-game neutral resolver, controller-sequence clip-time extrapolation, equal-priority sequence weighting, case-insensitive pose/remap/attachment bone lookup, controller-sequence blend-index routing, emissive RGB G-buffer transport, reflected caustic UVs, authored gloss multiplication and bump/luma bias, authored BSLighting specular strength and color, explicit BSShaderNoLighting unlit mode, neutral ESM light position/color/radius and flicker/pulse evaluation, negative-light subtraction, and point-light specular lighting | Add remaining dynamic shading |
| Terrain geometry and layer data | Renderer-neutral `Terrain::RenderStorage` contract with storage-owned decoded-land cell caching and grid-shift eviction, one-task asynchronous bounded LOD-0 tile prewarm plus lazily cached neutral tile snapshots, active-cell-aware aligned regions, root-relative quadtree block assembly, legacy texture-path correction, and a Vulkan opaque/normal/parallax/blendmap/specular layer consumer; concrete `MWRender::TerrainStorage` and legacy OSG ChunkManager remain the reference data path, including explicit ESM4 specular textures | Add quadtree-scale scheduling and broader image-format coverage |
| Loaded-cell object identity, transforms, terrain snapshots, and paging state | Renderer-neutral `WorldScene`/`CellScene` snapshots updated by scene lifecycle; active-cell static references bypass legacy OSG paging visibility, cell-lifecycle-cached terrain tiles flow into `SceneSubmission`, neutral terrain preloads consume predicted/grid/door/fast-travel bounds when the legacy cell preloader is absent, and static/dynamic/effect submissions apply neutral view-distance visibility; neutral movement, cell transfer, water, effect, and weather writes are now encapsulated by `MWWorld::Scene` | Vulkan consumes neutral snapshots; migrate remaining backend-owned mesh/paging policy |
...
remaining presentation-specific composition stays as follow-up work.
