...
namespace Vk
{
    ...
    VkSurfaceFormatKHR Swapchain::chooseSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& available) const
    {
        ...
        // An _SRGB format is required, not merely preferred: composite.frag writes linear values and
        // relies on the hardware to apply the transfer function on store. Both orderings are listed
        // because Android drivers commonly expose only R8G8B8A8.
        const std::array<VkFormat, 2> preferred = { VK_FORMAT_B8G8R8A8_SRGB, VK_FORMAT_R8G8B8A8_SRGB };
        ...
        Log(Debug::Warning) << "Vulkan: no sRGB surface format available, falling back to format "
                            << static_cast<int>(available[0].format) << " -- output will be too dark";
        return available[0];
    }
